...
export async function mergeIntoOne(
  ...
  try {
    ...
    const outputFilePath = join(dir, "../S2.pdf");
    ...
    // meta overrides
    pdfDoc.setAuthor("Sahithyan's S2 (https://s2.sahithyan.dev)");
    ...
    await writeFile("./dist/S2.pdf", await pdfDoc.save()).then(() =>
      logger.info("saved: ./dist/S2.pdf"),
    );
  } catch (error) {
...
export async function createPrefaceDoc(
  ...
  pdfDoc.setProducer("Sahithyan's S2 (https://s2.sahithyan.dev)");
